<template>
                ...
                <div class="box-body">
                    <div class="address">
                        ...
                        <div class="action">
                            ...
                            <el-button size="large" @click="addAddressFlag">添加地址</el-button>
                        </div>
                    </div>
                </div>
                <!-- 商品信息 -->
    ...
    <el-dialog v-model="toggleFlag" title="切换收货地址" width="30%" center>
        <div class="addressWrapper">
            <div class="text item" :class="{ active: activeAddress?.id === item.id }" v-for="item in checkInfo.userAddresses" :key="item.id">
        ...
        <div style="height: 300px; overflow-y: auto">
            <el-form :model="userForm" :inline="true" size="default">
                ...
                <el-form-item label="地址标签">
                    <el-select v-model="userForm.addressTags" class="m-2" placeholder="家，公司，学校" size="large">
                ...
                <el-form-item label="地址默认">
                    <el-select v-model="userForm.isDefault" class="m-2" placeholder="默认0:,不默认1" size="large">
...
import { reqCheckInfo, reqCreateOrder } from '@/api/checkout'
...
// 获取结算数据
const getCheckoutInfoAPI = async () => {
    let res = await reqCheckInfo()
    checkInfo.value = res.result
    console.log(res)
    const item = checkInfo.value.userAddresses.find((item: any) => item.isDefault === 0)
    curAddress.value = item
}

// 切换地址
const upDataAddresses = (item: any) => {
...
const submitOrder = async () => {
  const res = await reqCreateOrder({
    deliveryTimeType: 1,
    payType: 1,
    payChannel: 1,
    buyerMessage: '',
    goods: checkInfo.value.goods.map(
    ...
    addressId: checkInfo.value.userAddresses.filter(
